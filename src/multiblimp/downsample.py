...
def create_conditions(
    ...
    for condition in df.feature_vals.unique():
        ...
        for order in ["SV", "VS"]:
            ...
            child_order_count = sum(
                freq
                for (c, _, o), freq in counts.items()
                if (c == gram_val) and (o == order)
            )
            p_agreement = binom_test(
                counts[gram_key], n=child_order_count, p=p0, alternative="greater"
            )
            p_no_agreement = binom_test(
                counts[gram_key], n=child_order_count, p=p0, alternative="less"
            )
            ...
            if counts[gram_key] == 0:
                ...
                if p_agreement <= (alpha / 2):
                    asterisk = "+"
                elif (counts[gram_key] > min_total_pairs) and (
                    counts[gram_key] / child_order_count
                ) > p0:
                    asterisk = "~"
            ...
            if asterisk is None or (len(sub_df) < min_cond_pairs):
                if len(sub_df) > 0:
                    if asterisk is None:
                        print(
                            f"SKIPPING {len(sub_df)} items | low prob:",
                            lang_dir,
                            gram_val,
                            ungram_val,
                            order,
                        ...
                        print(
                            f"SKIPPING {len(sub_df)} items | little data:",
                            lang_dir,
                            gram_val,
                            ungram_val,
                            order,
                ...
                balanced_df = balanced_df.loc[
                    :, ~balanced_df.columns.str.contains("^Unnamed")
                ]
...
def set_attractors(df, ufeat, gram_val):
    ...
    df["congruent_attractors"] = [
        [attr for attr in attractors if allval2um(attr["feats"][ufeat]) == gram_val]
    ...
    df["only_has_congruent_attractors"] = (df.num_congruent_attractors > 0) & (
        df.num_incongruent_attractors == 0
    )
    df["only_has_incongruent_attractors"] = (df.num_incongruent_attractors > 0) & (
        df.num_congruent_attractors == 0
    )
