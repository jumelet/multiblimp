...
class Pipeline:
    ...
    def process_language(self, lang: str):
        ...
        inflector, skip_lang, num_lemma, num_form = self.load_inflector(
            lang, langcode, self.unimorph_inflect_args
        )
        ...
        if not skip_lang:
            ...
            context_inflector = self.load_inflector(
                lang, langcode, self.unimorph_context_args
            )[0]
        ...
        diagnostics = {
            item_type: pd.DataFrame(items) for item_type, items in diagnostics.items()
        }
