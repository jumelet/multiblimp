...
def load_pairs(pair_dir: str) -> dict[str, dict[str, pd.DataFrame]]:
    ...
    for csv_path in tqdm(csv_paths, desc="Loading CSVs"):
        ...
        data[lang][deprel] = pd.read_csv(csv_path, low_memory=False, usecols=KEEP_COLUMNS)
...
def upload(pair_dir: str, dataset_name: str) -> None:
    ...
    api = HfApi()
    api.create_repo(repo_id=dataset_name, repo_type="dataset", private=True, exist_ok=True)

    for lang, deprel_map in tqdm(sorted(lang_data.items()), desc="Uploading subsets"):
        subset = DatasetDict(
            {deprel: Dataset.from_pandas(df, preserve_index=False) for deprel, df in deprel_map.items()}
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Upload word-order pairs to HuggingFace")
    parser.add_argument("pair_dir", help="Root directory containing {deprel}/{lang}.csv files")
    parser.add_argument("dataset_name", help="HuggingFace dataset name, e.g. 'username/my-dataset'")
